...
function GuiEditorCtrlProperties::updateDynamicFields(%this)
{
	...
	for(%i = 0; %i < %count; %i++)
	{
		...
		%val.setFieldValue("AltCommand", "GuiEditorCtrlProperties.setData(" @ %label.getId()  @ ".getText()," @ %val.getId() @ ".getText());");
		%val.setFieldValue("Validate", "GuiEditorCtrlProperties.setData(" @ %label.getId()  @ ".getText()," @ %val.getId() @ ".getText());");
